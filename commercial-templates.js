(() => {
  ...
  function init(){
    ...
    const count=document.getElementById('templateResultCount'); const productionCount=document.getElementById('productionTemplateCount'); const empty=document.getElementById('templateEmpty');
    let filter='all';
    const refresh=()=>{const q=(search?.value||'').trim().toLowerCase();let visible=0;cards.forEach(card=>{const hay=`${card.dataset.templateTags} ${card.textContent}`.toLowerCase();const show=(filter==='all'||hay.includes(filter))&&(!q||hay.includes(q));card.hidden=!show;card.style.display=show?'':'none';if(show)visible++;});if(productionCount)productionCount.textContent=visible;if(count)count.textContent=`${visible} ready design${visible===1?'':'s'}`;if(empty)empty.hidden=visible!==0;};
